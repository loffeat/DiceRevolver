...
namespace DiceRevolver.Tests
{
    public sealed class EventRuleLightningMigrationTests
    {
        ...
        [Test]
        public void LegacyLightningControlsRemainAvailableBeforeCleanup()
        {
            ProjectileSpawnEffect orb = Load<ProjectileSpawnEffect>(
                $"{Root}/BulletEvents/FireLightningOrbProjectile.asset");
            ElectromagneticResonanceEffect resonance = Load<ElectromagneticResonanceEffect>(
                $"{Root}/BulletEvents/ElectromagneticResonanceEffect.asset");
            TeslaPassiveEffect tesla = Load<TeslaPassiveEffect>(
                $"{Root}/BulletEvents/TeslaPassiveEffect.asset");
            EchoSynergyPassiveEffect echo = Load<EchoSynergyPassiveEffect>(
                $"{Root}/BulletEvents/EchoSynergyPassiveEffect.asset");

            Assert.That(orb.PrimaryProjectile, Is.True);
            Assert.That(orb.ProjectileDefinition, Is.SameAs(Load<ProjectileDefinition>(
                $"{Root}/Projectiles/LightningOrb.asset")));
            Assert.That(resonance.SearchRadius, Is.EqualTo(6f));
            Assert.That(resonance.MaximumConnections, Is.EqualTo(3));
            Assert.That(tesla.DamagePerStack, Is.EqualTo(0.05f));
            Assert.That(echo.MaximumTriggersPerChamber, Is.EqualTo(4));
            Assert.That(echo.MaximumSpreadAngle, Is.EqualTo(8f));
            Assert.That(echo.MinimumSpreadSeparation, Is.EqualTo(2f));
        }
