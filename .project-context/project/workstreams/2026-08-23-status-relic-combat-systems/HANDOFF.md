...
- `active`：五系统 T1–T8 + 用户后续需求已实现；呼应协同真实 Gun 链路与右上角骰面 HUD 已修复。收尾者覆盖行为已按用户要求回档，当前仍需后续正常 Play Mode 观察与占位参数调优。
...
- 保护槽位：事件规则可数据化设置“保护槽位免受覆盖”，策略位于通用快照合并层且不硬编码事件名称；当前所有现有事件均保持修改前配置，没有默认保护，收尾者可被链式反应覆盖。

## 尚未完成

- 修复后的骰面 HUD、呼应协同（含面 4 链式反应）、收尾者回档后的覆盖表现、特斯拉增伤和点燃表现的正常 Play Mode 验收。
- 根据手感决定是否调整特斯拉 `0.05`、点燃 `3 秒 / 2 dps` 与弹丸占位立绘；未验收前不把这些参数视为正式定稿。

## 下一步首个动作

- 在后续正常 Play Mode 中先按用户截图构筑复验：带基础事件的链式反应排到收尾者之前时，最后一发应使用被复制的基础弹丸；随后观察骰面 HUD、呼应协同、特斯拉和点燃表现。
...
- [passed] `2026-08-24`：事件配置按用户要求回档；隔离 Unity 完成 RED `1/3` → GREEN `3/3`，确认收尾者恢复可覆盖，同时显式保护能力仍有效。

## 风险与不可假定事项

- 相邻触发含面 4（D4/D5）→ 链式反应排队可能造成额外循环，PlayMode 待验收。
- “保护槽位免受覆盖”目前没有现有规则开启；未来若某条规则需要保留自身槽位，应在规则资产中显式启用，不应按名称写特殊判断。
